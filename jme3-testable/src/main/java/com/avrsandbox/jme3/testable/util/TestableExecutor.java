package com.avrsandbox.jme3.testable.util;
...
/**
 * A Utility for searching Testable classes with test tags {@link TestableTags#value()}
...
public final class TestableExecutor {
    ...
    /**
     * Searches some packages for {@link Testable}s with some {@link TestableTags} and launches them synchronously.
     *
     ...
     * @param tags     the testables classes tags {@link TestableTags#value()}
    ...
    /**
     * Searches a java package for some {@link Testable}s with some {@link TestableTags} and launches them synchronously.
     *
    ...
    /**
     * Executes {@link Testable#launch(Object)} of a Testable class (clazz) with a user param (userData) and tags {@link TestableTags#value()}.
     *
     * @param clazz    a testable class
     * @param userData the {@link Testable#launch(Object)} parameter object
     * @param tags     class annotation tags {@link TestableTags#value()}
     */
    ...
    private boolean hasOneOfClassTags(Class<?> clazz, String[] tags) {
        for (Annotation annotation : clazz.getDeclaredAnnotations()) {
            ...
            if (!(annotation instanceof TestableTags)) {
                continue;
            }
            // return if one of the user tags is true
            for (String tag : tags) {
                // compare all values of the annotation
                for (String value : ((TestableTags) annotation).value()) {
                    if (value.equals(tag)) {
                        return true;
                    }
                }
